...
bdl.geodash.MarkerEditor = Backbone.View.extend({
	...
	render : function() {
		...
		this.$(".gd-iconType").button().bind("click", function() {
			self.iconPicker();
		});
	...
	iconPicker : function() {
		...
		$(".gd-iconTypePreview", self.iconDialog).button().bind("click", function(e) {
	...
	template : _.template('<form id="layer-editor">' + '<input type="hidden" name="type" value="<%= type %>" />' + '<div class="gd-icon-picker"> </div>' + '<div class="gd-editor-title">Marker Layer Editor</div> ' + '<div class="gd-editor-notice ui-state-error ui-corner-all"><span class="ui-icon ui-icon-alert"></span><span class="gd-notice"></span></div>' + '<div class="gd-editor-section"> ' + '<div class="gd-section-label">Source</div>' + '<div class="gd-section-options">'
			...
			'	<div class="gd-ge gdgrid-report" style="<% if(source != "gdgrid"){ print("display:none;"); } %>">' + '		<div class="gd-u first">' + '			<label for="reportID">Grid ID:</label><br />' + '			<select id="gdgrid" name="gdGridId">' + '				<% _.each(gdGridBones,function(val,idx){ %>' + '					<option value="<%= val.id %>" <% if(val.id == gdGridId ){ print("SELECTED"); } %> ><%= mstrApp.isVI? val.parent.parent.title:val.n %></option>' + '				<% }); %> ' + '			</select>' + '		</div>'
			...
			+ '					<option value="<%= idx %>" <% if(idx==keys.title){ print("SELECTED"); } %> ><%= val.name %></option>' + '				<% }); %> ' + '			</select>' + '		</div>' + '	</div>	' + '</div>' + '</div>' + '		<div class="gd-editor-section"> ' + '<div class="gd-section-label">Marker</div>' + '<div class="gd-section-options">' + '	<div class="gd-editor-item-desc">' + '		Click the icon shown below to choose a different marker.' + '	</div>' + '	<div class="gd-ga">' + '		<div class="gd-u first">'
			+ '			<input type="hidden" value="<%= iconType %>" id="iconType" name="iconType"/>' + '			<label for="iconType">Icon Type: </label>' + '		</div>' + '<div class="gd-u gd-icon-option">	' + '			<div class="gd-iconType" value="dynamic-pin">' + '				<img src="<%= thumb %>" />' + '			</div>						' + '		    <div>dynamic-pin</div>' + '		</div>' + '	</div>' + '	<div class="gd-editor-item-desc">'
			+ '		Static color applies the same color to all records while threshold colors applies a unique color based on the underlying grid cell.' + '	</div>' + '	<div class="gd-ga">	' + '		<div class="gd-u first">' + '			<label for="colorType">Color: </label>' + '		</div>' + '		<div class="gd-u gd-radio">	' + '			<input value="static" id="colorType-static" name="colorType" type="radio" <% if(colorType == "static"){ print("checked"); } %> /><label for="colorType-static">Static</label> '
			+ '			<input value="threshold" id="colorType-thresh" name="colorType" type="radio" <% if(colorType == "threshold"){ print("checked"); } %>/><label for="colorType-thresh">Thresholds</label> ' + '		</div>' + '	</div>' + '	<div class="gd-ga gd-staticColor" style="<% if(colorType == "threshold"){ print("display:none;"); } %>" >' + '		<div class="gd-u first">' + '			<label for="staticColor">Static Color: </label>' + '		</div>' + '		<div class="gd-u">	'
			+ '			<div class="colorSelector" id="staticColorPicker"><div style="background-color: <%= staticColor %>"></div></div>' + '			<input type="hidden" value="<%= staticColor %>" id="staticColor" name="staticColor" />' + '		</div>' + '	</div>' +
			...
			+ '	<div class="gd-ga minMarkerWidth">	' + '		<div class="gd-u first">' + '			<label for="minMarkerWidth">Min Width: </label>' + '		</div>' + '		<div class="gd-u">' + '			<input type="text" id="minMarkerWidth" name="minMarkerWidth" value="<%= minMarkerWidth %>"/> ' + '		</div>' + '	</div>	' + '	<div class="gd-ga maxMarkerWidth">	' + '		<div class="gd-u first">' + '			<label for="maxMarkerWidth">Max Width: </label>' + '		</div>' + '		<div class="gd-u">'
			...
			'<div class="gd-editor-section"> ' + '<div class="gd-section-label">Concentric Distance Circles</div>' + '<div class="gd-section-options">' + '	<div class="gd-editor-item-desc">' + '		Draw concentric with a diameter of given distance circles around markers.' + '	</div><br/>' +

			'	<div class="gd-ge" >' + ' <div class="circles"></div> ' +

			' <div id="addcircle">add crcle</div> ' +

			' <div class="blankCircle">' + '		<div class="circle">' + '			<label for="">Color:</label><br />' + '			<div class="colorSelector" id="staticColorPicker inline"><div style="" class="color"></div></div>' + '			<label for="">Size:</label><br />' + '			<input type="text" class="circleSize" id="infoWindowWidth" name="circle.size" /> ' + '			<select name="circle.units" class="circleUnits" ><option>km</option><option>miles</option></select><br/>'
			+ '			<input type="hidden" value="" id="staticColor" name="circle.color" class="circleColor" />' + '			<div class="deleteCircle">remove</div>' + '		  <hr/>' + '		</div>' + '	 </div>' +
			...
			+ '			<input value=false id="active-n" name="on" type="radio" <% if(!on){ print("checked"); } %> /><label for="active-n">No</label> ' + '		</div>' + '	</div>' + '	<div class="gd-editor-item-desc">' + '		Clustering improves performances when displaying hundreds of markers.' + '	</div>' + '	<div class="gd-ga">' + '		<div class="gd-u first">' + '			<label>Cluster: </label>' + '		</div>' + '		<div class="gd-u gd-radio">	'
			+ '			<input value=true id="cluster-y" name="cluster" type="radio" <% if(cluster){ print("checked"); } %> /><label for="cluster-y">Yes</label> ' + '			<input value=false id="cluster-n" name="cluster" type="radio" <% if(!cluster){ print("checked"); } %> /><label for="cluster-n">No</label> ' + '		</div>' + '	</div>' + '</div>' + '</div></form>' + '<div class="gd-dialog infoWindow"><dl>' + '<dt>Default</dt><dd>Attributes and their values will be displayed.</dd>'
