...
bdl.geodash.MarkerEditor = Backbone.View.extend({
	tagName: "div",
    ...
    if(this.p.gd.base.is('google')) {
      m.thumb = bdl.geodash.IconFactory.createIcon({
        primaryColor: "#ff9900",
        type: this.model.get("iconType"),
        useDefaultSize: true
      }).thumb;
    } else if(this.p.gd.base.is('leaflet-mapquest')) {
      m.thumb = bdl.geodash.IconFactory.leafletCreateMarkerIcon({
        primaryColor: "#f90",
        type: this.model.get("iconType"),
        useDefaultSize: true
      }).options.thumb;
    ...
    function showHideButtons() {
      ...
      showHideButtons();
		...
		this.$(".gd-iconType").button()
		.bind("click",function(){self.iconPicker();});
		...
		this.$("#addcircle").button()
		.bind("click",function(){self.addCircle();});
  ...
  addCircle: function(opts) {
    var defaults = {color: '66dd66', size: 2, units: 'miles'};
  ...
  setVisibilityOfcolorOptions: function() {
	...
	iconPicker: function(){
		...
		this.iconDialog = this.$(".gd-icon-picker").dialog({
			modal: true,
			width: 320,
			height: 275,
			title: "Icon Picker",
		...
		var self = this;
		_.each(bdl.geodash.IconFactory.ICONS,function(val,key){
      if(self.p.gd.base.is('google')) {
        var icon = bdl.geodash.IconFactory.createIcon({
          primaryColor: "#ff9900",
          type: key,
          useDefaultSize: true
        });
        $(self.iconDialog).append(self.iconTemplate(icon));
      } else if(self.p.gd.base.is('leaflet-mapquest')) {
        var icon = bdl.geodash.IconFactory.leafletCreateMarkerIcon({
          primaryColor: "#ff9900",
          type: key,
          useDefaultSize: true
        });
        $(self.iconDialog).append(self.iconTemplate(icon.options));
      }
		});
		$(".gd-iconTypePreview",self.iconDialog).button()
			.bind("click",function(e){
				var val = $(this).next("div").text();
				self.$("input[name=iconType]").val(val);
				var img = $(e.currentTarget).find("img").attr("src");
				self.$(".gd-iconType").next("div").text(val);
				self.$(".gd-iconType img").attr("src",img);
				$(self.iconDialog).dialog("close");
        self.setVisibilityOfcolorOptions();
			});
	},
	
	iconTemplate: _.template(
		'<div class="gd-u"><button class="gd-iconTypePreview" value="<%= type %>" >'+
			'<img src="<%= thumb %>" />'+
		'</button><div><%= type %></div></div>'),
	
	template: _.template(
		'<form id="layer-editor">'+
		'<input type="hidden" name="type" value="<%= type %>" />'+ 
		'<div class="gd-icon-picker"> </div>' +
		'<div class="gd-editor-title">Marker Layer Editor</div> '+
		 '<div class="gd-editor-notice ui-state-error ui-corner-all"><span class="ui-icon ui-icon-alert"></span><span class="gd-notice"></span></div>'+
			...
			'<div class="gd-section-label">Source</div>'+
			...
			'		<input value="external" id="source-external" name="source" type="radio" <% if(source == "external"){ print("checked"); } %> /><label for="source-external">External Report</label> '+
			...
			'</div>'+
			'		<div class="gd-editor-section"> '+
			'<div class="gd-section-label">Marker</div>'+
			'<div class="gd-section-options">'+
			'	<div class="gd-editor-item-desc">'+
			'		Click the icon shown below to choose a different marker.'+
			'	</div>'+
			'	<div class="gd-ga">'+
			'		<div class="gd-u first">'+
			'			<input type="hidden" value="<%= iconType %>" id="iconType" name="iconType"/>'+
			'			<label for="iconType">Icon Type: </label>'+
			'		</div>'+
					'<div class="gd-u gd-icon-option">	'+
			'			<div class="gd-iconType" value="dynamic-pin">'+
			'				<img src="<%= thumb %>" />'+
			'			</div>						'+
			'		    <div>dynamic-pin</div>'+
			'		</div>'+
			'	</div>'+
			'	<div class="gd-editor-item-desc">'+
			'		Static color applies the same color to all records while threshold colors applies a unique color based on the underlying grid cell.'+
			'	</div>'+
			'	<div class="gd-ga">	'+
			'		<div class="gd-u first">'+
			...
			'			<input value="static" id="colorType-static" name="colorType" type="radio" <% if(colorType == "static"){ print("checked"); } %> /><label for="colorType-static">Static</label> '+
			'			<input value="threshold" id="colorType-thresh" name="colorType" type="radio" <% if(colorType == "threshold"){ print("checked"); } %>/><label for="colorType-thresh">Thresholds</label> '+
			'		</div>'+
			'	</div>'+
			'	<div class="gd-ga gd-staticColor" style="<% if(colorType == "threshold"){ print("display:none;"); } %>" >'+
			'		<div class="gd-u first">'+
			'			<label for="staticColor">Static Color: </label>'+
			'		</div>'+
			'		<div class="gd-u">	'+
			'			<div class="colorSelector" id="staticColorPicker"><div style="background-color: <%= staticColor %>"></div></div>' +
			'			<input type="hidden" value="<%= staticColor %>" id="staticColor" name="staticColor" />'+
			'		</div>'+
			'	</div>'+

			'	<div class="gd-ga gd-thresh-metric">	'+
			'		<div class="gd-u first">'+
			'			<label for="keys.colorMetric">Color Metric: </label>'+
			'		</div>'+
			'		<div class="gd-u">	'+
			'			<select id="colorMetric" name="keys.colorMetric">'+
			'				<% _.each(columns,function(val,idx){ %>'+
			'					<option value="<%= idx %>" <% if(idx==keys.colorMetric){ print("SELECTED"); } %> ><%= val.name %></option>'+					
			'				<% }); %> '+
			'			</select>'+
			'		</div>'+
			'	</div>	'+
      ...
      ' <div id="size-options">' +
			'	<div class="gd-ga gd-thresh-metric">	'+
			'		<div class="gd-u first">'+
			'			<label for="keys.sizeMetric">Size Metric: </label>'+
			'		</div>'+
			'		<div class="gd-u">	'+
			'			<select id="sizeMetric" name="keys.sizeMetric">'+
			'				<% _.each(columns,function(val,idx){ %>'+
			'					<option value="<%= idx %>" <% if(idx==keys.sizeMetric){ print("SELECTED"); } %> ><%= val.name %></option>'+					
			'				<% }); %> '+
			'			</select>'+
			'		</div>'+
			'	</div>	'+
			...
			'		<div class="gd-u first">'+
			'			<label for="minMarkerWidth">Min Width: </label>'+
			'		</div>'+
			'		<div class="gd-u">'+
			'			<input type="text" id="minMarkerWidth" name="minMarkerWidth" value="<%= minMarkerWidth %>"/> '+
			'		</div>'+
			'	</div>	'+
			'	<div class="gd-ga maxMarkerWidth">	'+
			'		<div class="gd-u first">'+
			'			<label for="maxMarkerWidth">Max Width: </label>'+
			'		</div>'+
			'		<div class="gd-u">'+
			'			<input type="text" id="maxMarkerWidth" name="maxMarkerWidth" value="<%= maxMarkerWidth %>"/> '+
			'		</div>'+
			'	</div>	'+
      ' </div> ' +
			'</div>'+
			...
			'<div class="gd-editor-section"> '+
			'<div class="gd-section-label">Info Window</div>'+
			'<div class="gd-section-options">'+
			'	<div class="gd-editor-item-desc">'+
			'		Choose what to display in the info window that appears when a marker is clicked on.'+
			'	</div><br/>'+
			'	<div class="gd-radio"> '+
			'		<input value="default" id="infoWindow-default" name="infoWindow" type="radio" <% if(infoWindow == "default"){ print("checked"); } %>/><label for="infoWindow-default">Default</label> '+
			'		<input value="document" id="infoWindow-document" name="infoWindow" type="radio" <% if(infoWindow == "document"){ print("checked"); } %> /><label for="infoWindow-document">Document URL</label> '+
			'		<input value="none" id="infoWindow-none" name="infoWindow" type="radio" <% if(infoWindow == "none"){ print("checked"); } %> /><label for="infoWindow-none">None</label> '+
			'		<button class="gd-help-button" gd-help-dialog="infoWindow" title="Help">Help</button>'+
			'	</div>'+
			'	<div class="customInfoWindowOptionals" style="<% if(infoWindow != "document"){ print("display:none;"); } %>" >' +
			'	<div class="gd-ge infoWindowDocumentURL" >' +
			'		<div class="gd-u first">'+
			'			<label for="infoWindowDocumentURL">Document URL:</label>'+
			'			<button class="gd-help-button" gd-help-dialog="documentUrl" title="Help">Help</button><br />'+
			'	    <textarea id="infoWindowDocumentURL" name="infoWindowDocumentURL" rows="4" cols="40" ></textarea>'+
			'		</div>'+
			'	</div>'+
			'	<div class="gd-ge infoWindowWidth" >' +
			'		<div class="gd-u first">'+
			'			<label for="infoWindowWidth">Width:</label><br />'+
			'			<input type="text" id="infoWindowWidth" name="infoWindowWidth" value="<%= infoWindowWidth %>"/> '+
			'		</div>'+
			'	</div>'+
			'	<div class="gd-ge infoWindowHeight" >' +
			'		<div class="gd-u first">'+
			'			<label for="infoWindowHeight">Height:</label><br />'+
			'			<input type="text" id="infoWindowHeight" name="infoWindowHeight" value="<%= infoWindowHeight %>"/> '+
			'		</div>' +
			'	</div>' +
      ' </div>' +
			'</div>' +
			...
			'<div class="gd-editor-section"> '+
			'<div class="gd-section-label">Concentric Distance Circles</div>'+
			'<div class="gd-section-options">'+
			'	<div class="gd-editor-item-desc">'+
			'		Draw concentric with a diameter of given distance circles around markers.'+
			'	</div><br/>'+
      ...
      ' <div id="addcircle">add crcle</div> ' +

			' <div class="blankCircle">'+
			'		<div class="circle">'+
			'			<label for="">Color:</label><br />'+
			'			<div class="colorSelector" id="staticColorPicker inline"><div style="" class="color"></div></div>' +
			'			<label for="">Size:</label><br />'+
			'			<input type="text" class="circleSize" id="infoWindowWidth" name="circle.size" /> '+
			'			<select name="circle.units" class="circleUnits" ><option>km</option><option>miles</option></select><br/>'+
			'			<input type="hidden" value="" id="staticColor" name="circle.color" class="circleColor" />'+
			'			<div class="deleteCircle">remove</div>'+
			'		  <hr/>'+
			'		</div>'+
			'	 </div>'+
      ...
      ' </div>' +
			...
			'	</div>'+
			'	<div class="gd-editor-item-desc">'+
			'		Clustering improves performances when displaying hundreds of markers.'+
			'	</div>'+
			'	<div class="gd-ga">'+
			'		<div class="gd-u first">'+
			'			<label>Cluster: </label>'+
			'		</div>'+
			'		<div class="gd-u gd-radio">	'+
			'			<input value=true id="cluster-y" name="cluster" type="radio" <% if(cluster){ print("checked"); } %> /><label for="cluster-y">Yes</label> '+
			'			<input value=false id="cluster-n" name="cluster" type="radio" <% if(!cluster){ print("checked"); } %> /><label for="cluster-n">No</label> '+
			'		</div>'+
			'	</div>'+
      '</div>'+
      '</div></form>' +
      '<div class="gd-dialog infoWindow"><dl>' +
      '<dt>Default</dt><dd>Attributes and their values will be displayed.</dd>' +
      '<dt>Document URL:</dt><dd>Allows you to specify a custom URL to load in the window.</dd>' +
      '</dl></div>' +
      '<div class="gd-dialog documentUrl">' +
      '<p><strong>URL</strong><br/>Paste in a URL to a dashboard, or a URL to an external site.</p>' +
      '<p><strong>Attributes</strong><br/>Currently selected attributes will be appended to the URL using the <code>elementsPromptAnswers</code> parameter.</p>' +
      '<p><strong>Hiding Sections</strong><br/>Append the following to the URL to hide the sections around a dashboard <code>&hiddenSections=header,path,dockTop,dockLeft,footer</code></p>' +
      '</div>'
	)
});
