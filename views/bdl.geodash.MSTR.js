...
bdl.geodash.MSTR = {
	...
	loadModel : function(opts) {
		...
		if (opts.model.get('source') == 'gdgrid') {
			...
			if (!taskInfo.gridKey) {
				var gdGridId = opts.model.get('gdGridId');
				if (gdGridId.indexOf("grid_") != -1) {
					taskInfo.gridKey = gdGridId.substring(gdGridId.indexOf('grid_') + 5, gdGridId.indexOf('_0'));
				} else {
					taskInfo.gridKey = mstrmojo.all[gdGridId].parent.k
				}
			}
		}
