...
bdl.geodash.MSTR = {
    ...
    getMarkerTileStage: function(opts, x, y, z) {
      ...
      stage.data = stage.data + "&" + $.param({x: x, y: y, z: z})
      return stage;
    },
      ...
      // console.log('MSTR.loadModel', 'stage', stage, 'stage.data', stage.data, opts.model);
			$.post(stage.url,stage.data,function(resp){ hs(resp); },'json')
			.error(function(resp){ he(resp); } )
			.complete(function(resp){ a.updateActionStage(resp); });
		},
      ...
      data= "taskId=reportExecute&taskEnv=xhr&taskContentType=json&styleName=GeodashActionStyle&geodashAction=getLayerColumns" +
        "&reportID=" + opts.model.get('reportID')+"&sessionState=" + form.find("input[name=sessionState]").val();
      ...
      $.post(url,data,function(resp){ hs(resp); },'json')
      .error(function(resp){ he(resp); } );
    },
      ...
      var mSrc = model.get('source');
			...
			var js = _.clone(model.toJSON());
			delete js["rows"];
			if(model.get('type') != "kmlLayer"){ delete js["url"]; }
			delete js["geom"];
      ...
      // console.log('form', form, form.attr('action'));
		...
		updateActionStage: function(data){
			try{ data = JSON.parse(data.responseText); }catch(e){ data = {}; };
