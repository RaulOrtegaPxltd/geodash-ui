/**
 * options - model - A Heatmap Layer object
 */
...
bdl.geodash.HeatmapEditor = Backbone.View.extend({
	...
	template : _.template('<form id="layer-editor">' + '<input type="hidden" name="type" value="<%= type %>" />' + '<div class="gd-editor-title">Heatmap Layer Editor</div> ' + '<div class="gd-editor-notice ui-state-error ui-corner-all"><span class="ui-icon ui-icon-alert"></span><span class="gd-notice"></span></div>' + '<div class="gd-editor-section"> ' + '<div class="gd-section-label">Options</div>' + '<div class="gd-section-options">' + '	<label for="name">Layer Name:</label>'
			...
			'		<div class="gd-editor-section"> ' + '<div class="gd-section-label">Location</div>' + '<div class="gd-section-options">' + '	<div class="gd-editor-item-desc">' + '		Choose the field containing a lat,lng pair or the address. ' + '	</div>' + '	<div class="gd-ga">' + '		<div class="gd-u first">' + '			<label for="keys.geo">Geocode: </label>' + '		</div>' + '		<div class="gd-u">	' + '			<select id="geo" name="keys.geo">' + '				<% _.each(columns,function(val,idx){ %>'
			...
			+ '					<option value="<%= idx %>" <% if(idx==keys.title){ print("SELECTED"); } %> ><%= val.name %></option>' + '				<% }); %> ' + '			</select>' + '		</div>' + '	</div>	' + '</div>' + '</div>' +

			'		<div class="gd-editor-section"> ' + '<div class="gd-section-label">Shape Options</div>' + '<div class="gd-section-options">' + '	<div class="gd-editor-item-desc">' + '  A weighted metric gives the heatmap more intensity for higher values' + '	</div>' + '	<div class="gd-ga gd-thresh-metric">	' + '		<div class="gd-u first">' + '			<label for="weightMetric">Weighted metric: </label>' + '		</div>' + '		<div class="gd-u">	' + '			<select id="weightMetric" name="weightMetric">'
			+ '	  		<option value="-1" <% if(-1==weightMetric){ print("SELECTED"); } %> >None</option>' + '				<% _.each(columns,function(val,idx){ %>' + '					<option value="<%= idx %>" <% if(idx==weightMetric){ print("SELECTED"); } %> ><%= val.name %></option>' + '				<% }); %> ' + '			</select>' + '		</div>' + '	</div>	' +

			'	<div class="gd-editor-item-desc">' + '   The size in pixels for the heatmap points.' + '	</div>' + '	<div class="gd-ga">	' + '		<div class="gd-u first">' + '			<label for="radius">Radius: </label>' + '		</div>' + '		<div class="gd-u">	' + '			<input type="text" value="<%= radius %>" id="radius" name="radius" />' + '		</div>' + '	</div>' + '</div>' + '</div>' +
			...
			+ '			<input value=false id="active-n" name="on" type="radio" <% if(!on){ print("checked"); } %> /><label for="active-n">No</label> ' + '		</div>' + '	</div>' + '</div>' + '</div></form>')
});
