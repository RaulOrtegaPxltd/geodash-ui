...
bdl.geodash.AreaView = Backbone.View.extend({
    ...
    if(this.model.get('infoWindow') != "none") {
      if(isNav){
        ...
        if (this.map.base.is('google')) {
          if(this.currentHighlite){ this.currentHighlite.setMap(null);}
          this._geocodedIBox(row);
        } else if (this.map.base.is('leaflet-mapquest')) {
          if(this.currentHighlite){ this.map.apiMap.removeLayer(this.currentHighlite) };
          this._geocodedIBox(row);
        }
      }else{
	...
	renderLayer: function(){
      ...
      this.map.gd.log({type:"info",message: "Loading tiles..."});
      var self = this;

      var handleClick = function(e){
        self.ib.close();
        self.map.gd.log({type:"info",message: "Getting info window for area."});
        if(e.latLng){
      ...
      var geodash_layer = new gdLeafletLayer(gd_url, {
        attribution: '',
        maxZoom: 22 //this.model.get("max_zoom")
      });
    ...
    if(this.model.get('rows').length == 0 && state != "not_ready") {
      this.tab.setError({message: "No data to display."});
            ...
            // console.log('AreaView.load model:', self.model, ' layerKey', self.model.get('layerKey'));
						...
						self.tab.setError({message: self.model.get('errors')[0]}); 
					}					
				},
